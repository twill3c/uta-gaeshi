...
| L3 | 名寄せ表と三段台帳の確定 | 完了 |
| L4 | 和訳(継続中) | 第1〜12巻 完訳・全体 1,247/2,432(51.3%) |
| L5 | 較正ゲートの測定 | 未着手 |
...
一度入った中核句は以後のすべての巻を拘束する。現在 557 件。
既存の用語集で足りた割合は単調には上がらない。第7巻の 56% が最高で、
第9巻は 31% まで落ちた。**巻の内容が変わると累積の効きが戻る** —
第9巻はオデュッセウス自身の一人称の航海譚で、出航・上陸・出発の定型が
まとめて新規に入るため。累積は「進むほど楽になる」性質のものではない。
ただし第10巻は 54%、第12巻は **70% と最高**まで戻った。語り手が同じ一人称の
まま続くかぎり、第9巻で払った新設の費用がそのまま効きつづける。
**落ちるのは巻ではなく語りが変わるとき**である。放浪譚(第9〜12巻)は
0.31 → 0.54 → 0.51 → 0.70 と、巻を追うごとに回収が進んだ。
